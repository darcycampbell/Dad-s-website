...
function AboutPage() {
    ...
    function displayPrincipleText(event) {
        let selected;
        principles.forEach((each) => 
            {
                if (each.name === event.target.innerText) {
                    selected = each;
                }
            }
        );
        ...
        textBoxElement.current.classList.add("textBoxStyle");
    }

    function hidePrincipleText() {
        setPrincipleText("");
        setDescriptionText("");
        textBoxElement.current.classList.remove("textBoxStyle");
    }

    return (
      <div>
        <div className="about textbox" ref={textBoxElement}>
          <h2>{principle}</h2> 
          <p>{description}</p>
        </div>
